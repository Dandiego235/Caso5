    ...
    while (true) {
        ...
        if (opcion == 1){
            while (true) { // Se pide la información del registro.
                ...
                std::cout << date << endl;
                // Subir al server
                //regs.registerUser(nickname, offer, demand, password, ltm->tm_mday, ltm->mon, ltm->tm_year); // se registra el nuevo usuario
                allrecords.push_back(new Registered(nickname, offer,demand,date)); // Se añade al vector con los registros
            }
